...
**Token:** ~~default Actions `GITHUB_TOKEN` should suffice (all public data). Verify on first `workflow_dispatch`; fallback = no-scope classic PAT as `ACCESS_TOKEN` secret (script reads generic `GH_TOKEN` env either way).~~ **Superseded 2026-08-11 — this was wrong.** `GITHUB_TOKEN` is *not* sufficient: it renders `{Contributed: 2}` instead of `10`. `ACCESS_TOKEN` (classic PAT, scopes `repo` + `read:user`) is required, and it must **not** be deleted to resolve a 401. See the comment on `GH_TOKEN` in `.github/workflows/build.yml`.
...
- **`{Contributed: N}` is public-only under `GITHUB_TOKEN`** (shows 2; the real count incl. private CloudlyIO repos is 10). Workflow reads `secrets.ACCESS_TOKEN || secrets.GITHUB_TOKEN` — adding a repo-scoped PAT as the `ACCESS_TOKEN` secret upgrades the number, no code change needed. Contributions (539) already includes private activity because the account has "include private contributions" enabled.
  - **Follow-up 2026-08-11:** that PAT is therefore load-bearing, not an optional upgrade. It expired 30 days after creation and broke the daily build for two days (`HTTP 401 Bad credentials`). `||` does *not* fall through on an expired secret — only on an absent/empty one — so deleting the secret would turn the build green while silently reverting `Contributed` to 2. Use `Contributed == 10` as the acceptance test for any replacement token; the contributions total can't discriminate, since the "include private contributions" setting noted above may expose it either way.
